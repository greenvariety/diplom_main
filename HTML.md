...
## ~~1. Где плюс чтобы добавить группу~~

```html
<div class="card-head" style="outline: rgb(231, 76, 60) solid 2px; background-color: rgba(231, 76, 60, 0.08);"><div class="title">Группы<span class="muted" style="font-weight: 700;">: 1</span></div></div>
```
...
## 3. Здесь просто нужно добавлять какие предметы ведет и все! Уже на группе добавляются предметы и выбирается препод!
...
## 4. Почему тут переход по ссылке и такое больштое поле? Вкладка должна быьтбь как везде!
...
## 5. Везде сделай просто плюс, без надписей!
...
## 6. ЗАчем нам это, убери это, зачем добавлять отдельно студента для предмета? Это и в преподах убери!
