## PLAN задача — Иконки логотипа округлены, длинное тире заменено

**Что сделано:**

Иконки логотипа:
- В CSS `.brand-cell .logo` и `.login-side .brand-row .logo` — `border-radius` изменён на `50%` (круглый).
- Во всех местах `auth.jsx` и `main.jsx` где логотип задаётся через inline-стили — добавлен `borderRadius: '50%'`.

Замена длинного тире:
- Во всех файлах проекта (25 файлов: `.jsx`, `.js`, `.css`, `.py`) символ `—` заменён на `-`.
- Затронуты: все компоненты фронтенда, модели, API-вьюхи, тесты.
- Фронтенд пересобран командой `npm run build`.

---

## PLAN задачи 1 и 2 выполнены — Форма регистрации и экран сид-фразы

**Что сделано:**

Экран регистрации (`RegisterScreen`):
- Заголовок изменён с «Регистрация владельца» на «Создание аккаунта владельца».
- Подзаголовок заменён на: «После регистрации вы получите секретную сид-фразу. Она нужна для восстановления доступа к аккаунту. Запишите её и храните в безопасном месте.»
- Убраны серые placeholder-подсказки из полей логина и ФИО.
- Убрана серая hint-подпись под полем логина («Латиница, цифры и подчёркивание · 3–20 симв.»).
- Метка поля «Имя (отображается в системе)» заменена на «ФИО».
- Добавлена валидация ФИО: только кириллица, минимум 2 слова.

Экран сид-фразы (`SeedPhraseScreen`):
- Заголовок «Ваша сид-фраза» заменён на «Запишите секретную фразу».
- Красный баннер-предупреждение убран; вместо него — серый подзаголовок: «Эти 12 слов показываются один раз. Потеря фразы означает полную потерю доступа к аккаунту.»
- Текст чекбокса изменён на: «Я сохранил(а) фразу в надёжном месте и принимаю все риски потери доступа.»
...
## PLAN задача 3 выполнена — Регистрация больше не создаёт организацию

**Что сделано:**

- `core/api_auth.py` — из `RegisterView` убраны строки, которые автоматически создавали `Institution` (с именем и кодом пользователя) и привязывали её к новому пользователю.
- Теперь после регистрации владелец получает чистый аккаунт без организации и сам создаёт организации через экран выбора организации.
- `Institution` убран из импортов `api_auth.py` — он там больше не нужен.
- `python manage.py check` — 0 ошибок.

---

## PLAN задача 4 выполнена — Очистка демо-данных и ребрендинг

**Что сделано:**

- Все пользователи и все данные удалены из базы (SQLite). База пустая.
- Убран блок «Демо-доступы» с кнопками owner1/admin1/teacher1 на странице входа.
- Убрана валидация «Слишком короткий пароль» на форме входа — теперь только «Введите пароль».
- Иконка в браузерной вкладке заменена с SVG-заглушки на `logo.png`.
- Надпись "У" во всех местах (форма входа, регистрации, сид-фразы, восстановления, экран организаций) заменена на логотип из `logo.png`.
- Название «Учёт студентов» везде заменено на «АИСК».
- Слоган изменён на: «Единая система учета студентов и преподавателей. Замена бумажных журналов и Excel-таблиц. Личные данные, статусы, перевод и отчисление, документы, журнал изменений — всё в одном месте.»
- Футер изменён с «© Колледж · Дипломная работа · 2026» на «© ГБПОУ МКАГ · Дипломная работа · Пушков Н. М. · Группа ИСиП-3-22 · 2026».
- Заголовок вкладки изменён с «frontend» на «АИСК».
- Фронтенд пересобран (`npm run build`).

---

## PLAN задача 4 выполнена — Убраны серые примеры (placeholder) из всех полей ввода

**Что сделано:**

Из всех форм-полей ввода убраны серые placeholder-подсказки с примерами:
- Поля факультета: «ФИТ» и «Факультет информационных технологий».
- Поля организации: «Колледж информатики и технологий» и «КИТ».
- Поля пользователя: «teacher2» (логин) и «Иванов Иван Иванович» (ФИО).
- Поля сотрудника/студента: телефон «+7 » (в двух формах).
- Поля предмета «Например: Базы данных» и должности «Например: Преподаватель».
- Поле документа «Паспорт, Аттестат…».
- Textarea причины удаления «Например: уволен, отчислен, дубликат…».
- Поле кода организации в `core/forms.py`: «Например: КГУ».
- Поле создания организации в `main.jsx`: «Название организации».

Оставлены без изменений: поля поиска («Поиск студента…», «Поиск по ФИО…»), выпадающие Combobox («Выберите…», «Начните вводить…») — они несут функциональный смысл, а не примеры.

Фронтенд пересобран (`npm run build` — успешно).

---

## PLAN задача 3 выполнена — Кнопка «Создать» в форме создания организации

**Что сделано:**

- `frontend/src/main.jsx` — у кнопки «Создать» в форме создания организации убрано `style={{ flex: 1 }}`. Раньше она растягивалась на всё доступное место, из-за чего была гигантской рядом с маленькой кнопкой «Отмена». Теперь обе кнопки одинакового размера (`btn-sm`).
- Фронтенд пересобран (`npm run build` — успешно).

---

## PLAN задача 1 выполнена — Блокировка латыни в ФИО и кириллицы в логине

**Что сделано:**

- `frontend/src/auth.jsx` — поле логина (`LoginScreen`): при вводе любой кириллический символ автоматически удаляется (через `replace(/[А-ЯЁа-яё]/g, '')`). Кириллицу нельзя вставить ни с клавиатуры, ни через буфер обмена.
- `frontend/src/auth.jsx` — поле ФИО (`RegisterScreen`): при вводе любой латинский символ автоматически удаляется (`replace(/[A-Za-z]/g, '')`).
- `core/forms.py` — `OwnerRegisterForm.clean_username()`: добавлена серверная проверка — если логин содержит кириллицу, форма возвращает ошибку. `clean_display_name()`: добавлена серверная проверка — если ФИО содержит латиницу, форма возвращает ошибку.

---

## PLAN задача 2 выполнена — Пикер организации показывается всегда при открытии приложения

**Что сделано:**

- `frontend/src/main.jsx` — `loadUser(afterSwitch=false)` упрощён: при первом запуске (не после переключения орг) всегда устанавливает `_showPicker: true`. Вся логика автопереключения убрана.
- `frontend/src/main.jsx` — `OrgPickerScreen.load()`: убрано автопереключение при 1 доступной организации для admin/teacher. Теперь список всегда показывается, пользователь выбирает сам.
- `frontend/src/main.jsx` — `OrgPickerScreen.pickOrg()`: добавлен параметр `isActive`. Если организация уже активна — сразу вызывает `onOrgSelected()` без повторного POST.
- `frontend/src/main.jsx` — для активной организации в списке owner'а кнопка «Текущая» (заблокированная) заменена на «Войти» (кликабельная).
- `frontend/src/main.jsx` — условия рендеринга `AppShell` упрощены: вместо трёх отдельных if — одно: `if (currentUser._showPicker || !currentUser.institution)`.
- Фронтенд пересобран (`npm run build` — успешно).

---

## PLAN задача 3 выполнена — Кнопка «Сохранить» в форме редактирования организации

**Что сделано:**

- `frontend/src/main.jsx` — у кнопки «Сохранить» в inline-форме редактирования организации убрано `style={{ flex: 1 }}`. Из-за него кнопка растягивалась на всё доступное пространство и выглядела гигантской рядом с маленькой «Отмена». Теперь обе кнопки одинакового размера (`btn-sm`).
- Фронтенд пересобран (`npm run build` — успешно).

---

## PLAN задача 4 выполнена (дополнение) — Убраны оставшиеся placeholder-примеры во фронтенде

**Что сделано:**

Предыдущая сессия убрала примеры из форм создания/редактирования. В этой сессии убраны оставшиеся примеры в React-компонентах:
- `frontend/src/auth.jsx` — `placeholder="owner1"` из поля логина на экране восстановления пароля.
- `frontend/src/auth.jsx` — `placeholder="слово N"` из 12 полей ввода сид-фразы (номера слотов и так видны слева).
- `frontend/src/screens.jsx` — `placeholder="Иванов Иван…"` из полей поиска по ФИО на экранах студентов, сотрудников и опекунов (3 вхождения).
- `frontend/src/screens.jsx` — `placeholder="Студент, группа, admin1, Дмитриева…"` из поля поиска в журнале изменений.

Оставлены: `placeholder="Поиск…"`, `placeholder="Начните вводить ФИО сотрудника…"` и аналогичные — они описывают что делать, а не показывают пример данных.
...
## PLAN задача 6 выполнена — Точные сообщения об ошибках при незаполненных полях

**Что сделано:**

- `frontend/src/modals.jsx` — `OrgDeleteConfirmModal`: улучшена логика ошибок при нажатии «Удалить навсегда». Теперь:
  - Нет ни пароля, ни сид-фразы → «Введите пароль и сид-фразу».
  - Нет пароля, сид-фраза введена → «Введите пароль».
  - Пароль введён, но нет сид-фразы → «Введите сид-фразу».
  - Пароли не совпадают → «Пароли не совпадают».
  - Сид-фраза введена частично → «Введите все 12 слов сид-фразы».
- `frontend/src/modals.jsx` — `StudentFormModal`: тост «Проверьте обязательные поля» заменён на конкретный список незаполненных полей, например «Введите: фамилию, факультет».
- `frontend/src/modals.jsx` — `DeleteConfirmModal`: если не заполнена причина И не проставлена галочка → «Укажите причину удаления и подтвердите действие». Иначе — соответствующее конкретное сообщение.
- Фронтенд пересобран (`npm run build` — успешно).
