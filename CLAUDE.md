...
1. Прочитай **`TASK.md`** в корне проекта.
2. Выполни все задачи с пометкой `[ ]` — одну за другой, по порядку.
3. После выполнения всех задач — полностью очисти `TASK.md` (оставь только заголовок-инструкцию).
4. Если задач нет — работай в штатном режиме.
