...
Каждый новый чат обязан проверить **все три файла** в этом порядке и взять одну задачу из первого непустого:

1. **TASK.md** — задачи по логике / бэкенду / общие
2. **HTML.md** — задачи по правке интерфейса (режим правки)
3. **PLAN.md** — план разработки (если ни TASK.md, ни HTML.md не пусты)
...
**Если TASK.md пуст** — проверь **`HTML.md`** (режим правки):

**Если HTML.md не пуст** (есть незачёркнутые задачи):
1. Возьми **одну** первую незачёркнутую задачу.
2. Прочитай HTML-сниппет в задаче — это `outerHTML` конкретного элемента прямо из браузера.
3. Найди этот элемент в `frontend/src/` (по тексту кнопки, классу, структуре). Ищи в `screens.jsx`, `modals.jsx`, `shell.jsx`.
4. Внеси правку согласно описанию задачи.
5. Собери фронтенд: `cd frontend && npm run build`.
6. **ОБЯЗАТЕЛЬНО** зачеркни задачу в HTML.md: `~~## 1. Название~~` — не удаляй строку, именно зачеркни. Без этого шага работа не считается завершённой.
7. Запиши в **`STATUS.md`** что было сделано.
8. Сделай git push и заверши работу.
...
## Режим правки интерфейса (HTML.md)

**Что это:** пользователь кликает на элемент прямо в браузере через встроенный пикер элементов — и `outerHTML` этого элемента автоматически копируется в новую задачу в `HTML.md`. Рядом пользователь пишет что нужно поменять.

**Как выполнять задачу из HTML.md:**
1. Прочитай описание задачи (что нужно сделать).
2. Посмотри на HTML-сниппет — он показывает точный элемент: тег, классы, текст, атрибуты. Красная обводка (`outline: rgb(231, 76, 60) solid 2px`) — это просто маркер из пикера, в коде её нет.
3. Найди этот элемент в JSX-файлах: ищи по тексту кнопки / уникальному классу / структуре тегов.
4. Внеси изменение, пересобери фронтенд (`npm run build`).
5. **ОБЯЗАТЕЛЬНО** зачеркни задачу в HTML.md: `~~## N. Название~~` — это обязательный шаг после каждой правки.

**Важно:** HTML-сниппет — это **собранный** HTML из `frontend/dist/`, а редактировать нужно **исходники** в `frontend/src/`. Ищи соответствующий JSX-код.
