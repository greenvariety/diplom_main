...
1. Найди связанные `.md` из `docs/` по теме задачи и прочитай их — там весь нужный контекст.
2. Выполни задачи одну за другой.
3. После выполнения каждой задачи — **немедленно удали её строку из TASK.md**. Не оставляй ничего: ни заголовков, ни оформления, ни пустых строк. Файл должен стать полностью пустым.
4. Запиши в **`STATUS.md`** простым языком (без терминов) что именно было сделано. Никаких «следующих шагов» — только факты о сделанном.
5. Сделай git push и заверши работу.
...
- После очистки файл должен быть абсолютно пустым (0 строк).

---

**Если TASK.md пуст** — открой **`PLAN.md`**:
...
- **Никаких `placeholder` в полях ввода.** Атрибут `placeholder` на `<input>` и `<textarea>` не добавлять — ни в JSX, ни в Django-шаблонах. Label над полем достаточно.
