## Автотесты написаны — 149 тестов по всем модулям

**Что сделано:**

Создана директория `core/tests/` с 16 файлами тестов и вспомогательным `helpers.py`.
Все тесты используют DRF `APIClient` с `force_authenticate` — адаптированы под JWT REST API.
Запуск: `python manage.py test core`. Все 149 тестов проходят.

Покрыто: модели, авторизация (JWT), выбор организации, права ролей, факультеты, группы, студенты, опекуны, сотрудники (мультиорг), должности, предметы, заявки на удаление, аудит-лог, дашборд, пользователи.
